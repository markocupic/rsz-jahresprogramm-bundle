...
class RszJahresprogrammListModuleController extends AbstractFrontendModuleController
{
    ...
    protected function getResponse(Template $template, ModuleModel $model, Request $request): ?Response
    {
        ...
        while ($objJahresprogramm->next())
        {
            $arrJahresprogramm[] = [
                ...
                'end_date'        => $this->datumswandler(date('Y-m-d', (int) $objJahresprogramm->end_date)),
    ...
    public function datumswandler(string $Datum): string
    {
        ...
        $Datum = $WochentagDeutsch . ', ' . $Tag . '.' . $Monat . '.' . $Jahr; //H�ngt die 3 Variablen zum fertigen Datum zusammen
        return $Datum;
    }
